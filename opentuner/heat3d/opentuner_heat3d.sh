    ...
    for size in "EXTRALARGE"; do
        ...
        for seed in "1234" "2024" "9999"; do
            ...
            if [[ -f "${expect}" ]]; then
                ...
                todo="PYTHONPATH=.. python3 heat3d_opentuner.py --dataset-size ${size} --random-seed ${seed} --test-limit 200 --technique ${technique} --csv-output ${expect}";
            fi
            echo $todo;
