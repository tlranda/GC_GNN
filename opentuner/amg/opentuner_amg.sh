...
#techniques=$( PYTHONPATH=.. python3 amg_opentuner.py -lt --size SM );
    ...
    for size in "SM" "XL"; do
        ...
        for seed in "1234" "2024" "9999"; do
            ...
            if [[ -f "${expect}" ]]; then
                ...
                todo="PYTHONPATH=.. python3 amg_opentuner.py --size ${size} --random-seed ${seed} --test-limit 200 --technique ${technique} --csv-output ${expect}";
            fi
            echo $todo;
            #eval $todo;
            if [[ $? -ne 0 ]]; then
