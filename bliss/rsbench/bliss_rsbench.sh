...
module remove nvhpc;
cd /home/trandall/GC_GNN/bliss/rsbench;
...
for size in "SM" "XL" ; do
    ...
    for seed in "1234" "2024" "9999" ; do
        date;
        todo="PYTHONPATH=.. python3 rsbench_bliss.py --max-calls 200 --size ${size} --seed ${seed} --csv-output bliss_results/${size}/bliss_seed_${seed}.csv;";
        echo "${todo}";
        eval "${todo}";
        if [[ $? -ne 0 ]]; then
