...
for size in "SM" "XL" ; do
    ...
    for seed in "1234" "2024" "9999" ; do
        date;
        echo "PYTHONPATH=.. python3 heat3d_bliss.py --max-calls 200 --size ${size} --seed ${seed} --csv-output bliss_results/${size}/bliss_seed_${seed}.csv;";
        PYTHONPATH=.. python3 heat3d_bliss.py --max-calls 200 --size ${size} --seed ${seed} --csv-output bliss_results/${size}/bliss_seed_${seed}.csv;
        if [[ $? -ne 0 ]]; then
