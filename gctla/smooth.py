...
print("Begin")
# Load dataset
og_df = pd.read_csv("oracles/SM/all_SM.csv").sort_values(by=["objective"]).reset_index(drop=True)
print("Data loaded")
diff_df = dcpy(og_df)
# Columns we're working with
target_cols = [_ for _ in diff_df.columns if re.match(r"p[0-9]+", _) is not None]
...
X_train, X_test, Y_train, Y_test = train_test_split(float_df[target_cols], float_df['objective'], test_size=0.2, random_state=42)
...
initial_mask = fill_mask(mean_repr)
print("Precompute all masks")
# Save us time calculating masks by precomputing them all once
all_masks = [fill_mask(row) for (idx, row) in diff_df.iterrows()]
def match_mask(mask):
    indices = []
...
# Debug only to test this function
#print("Match mean-only")
#check_match = match_mask(initial_mask) # Should only return 1 value
#print("Matches: ", check_match)

def jitter(mask, inplace=True):
    ...
    if ltrue == 0:
        ...
        rmask[best_to_jitter,ltrue-1] = True
    return rmask
# Debug only to check this function
#jittered_mask = jitter(initial_mask, inplace=False)
#check_matches = match_mask(jittered_mask) # Should only return 2 values
...
"""
def capacity(used_indices, lower=0, upper=len(diff_df)):
    avail_dict = dict()
...
last_jitter = []
last_jitter_idx = match_mask(initial_mask)
jittered_mask = jitter(initial_mask, inplace=False)
#altered_column = np.where(initial_mask.sum(axis=1)-jittered_mask.sum(axis=1) < 0)[0][0]
#expected_new_values = len(values_per_column[target_cols[altered_column]]) ** (jittered_mask[altered_column].sum()-2)
iteration = 0
iteration_limit = np.sum([len(_) for _ in values_per_column.values()]) - len(target_cols)
# Do-while with loop condition at bottom
while True:
    print(f"Iteration {iteration} uses mask {jittered_mask.sum(axis=1)}")
    # These come pre-sorted, but may fill gaps in previous array
    jitter_idx = match_mask(jittered_mask)
    ...
    #print(new_jitter_idx.T.tolist()[0])
    ...
    avail_capacity = capacity(last_jitter_idx)
    best_smoothness = []
    length_accounted = 0
    ...
    smooth_mean.append(np.mean(min_dist))
    smooth_std.append(np.std(min_dist))
    ...
    if iteration >= iteration_limit:
        break
    new_mask = jitter(jittered_mask, inplace=False)
    #altered_column = np.where(jittered_mask.sum(axis=1)-new_mask.sum(axis=1) < 0)[0][0]
    #expected_new_values = len(values_per_column[target_cols[altered_column]]) ** (new_mask[altered_column].sum()-2)
    jittered_mask = new_mask
    print()
smooth_mean = np.asarray(smooth_mean)
smooth_std = np.asarray(smooth_std)
